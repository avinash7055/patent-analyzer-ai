    ...
    return (
      <div className="card">
        <div className="empty-tab">
          <Grid3X3 size={32} className="empty-tab-icon" />
  ...
  const handleExportCSV = () => {
    ...
    features.forEach((f) => {
      ...
      const row = [
        ...
        ...priorArtIds.map(
          (pa) =>
            `"${(featureMapping[pa] || "N/A").replace(/"/g, '""')}"`
        ),
      ];
      csv += row.join(",") + "\n";
    });
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h2>
          <Grid3X3 size={18} style={{ verticalAlign: "middle", marginRight: 8, color: "var(--accent)" }} />
      ...
      <div className="matrix-legend">
        <span className="legend-item">
          <span className="legend-dot novel" />
          NO (Novel)
        </span>
        <span className="legend-item">
        ...
        <span className="legend-item">
          <span className="legend-dot match" />
          Exact Match
        </span>
      </div>
              ...
              return (
                <tr key={i}>
                  <td className="sticky-col feature-cell">
                    <strong className="feature-id">{fid}</strong>
